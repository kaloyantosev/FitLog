    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      {mealPlan && (
        <div className="p-6 rounded-3xl bg-surface-1 border border-border space-y-6 shadow-lg">
          ...
          {/* 1. CONSOLIDATED INGREDIENTS BREAKDOWN (Сбор за всички избрани ястия) */}
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-blue-500/10 to-purple-500/10 border border-emerald-500/20 flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <PackageCheck className="w-6 h-6 text-emerald-400 shrink-0" />
                <div>
                  <h4 className="text-sm font-bold text-white">
                    Обобщен списък на съставките за пазаруване: {consolidatedSelectedIngredients.totalWeightKg} кг ({consolidatedSelectedIngredients.totalUniqueItems} уникални продукта)
                  </h4>
                  <p className="text-xs text-text-muted">
                    Точните количества за {consolidatedSelectedIngredients.selectedMealsCount} избрани ястия. Кликнете върху съставката за отмятане в магазина.
                  </p>
                </div>
              </div>
              <div className="text-xs font-mono text-emerald-300 font-bold px-3 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/30 shrink-0">
                {consolidatedSelectedIngredients.selectedMealsCount} маркирани ястия
              </div>
            </div>

            {Object.keys(consolidatedSelectedIngredients.grouped).length === 0 ? (
              <div className="p-8 text-center rounded-2xl bg-surface-2/40 border border-border/60 text-text-muted text-xs">
                Няма избрани ястия. Изберете поне едно ястие от списъка по-долу или кликнете &quot;Избери всички ястия&quot;.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(consolidatedSelectedIngredients.grouped).map(([catKey, group]) => {
                  if (group.items.length === 0) return null;
                  return (
                    <div
                      key={catKey}
                      className="p-4 rounded-2xl bg-surface-2/60 border border-border/70 flex flex-col justify-between space-y-3"
                    >
                      <div>
                        <div className="flex items-center justify-between border-b border-border/40 pb-2 mb-3">
                          ...
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface-3 text-text-muted font-bold">
                            {group.items.length} вида
                          </span>
                        </div>

                        <div className="space-y-1.5">
                          {group.items.map((item) => {
                            const itemKey = `consolidated-${item.name}`;
                            const isChecked = !!groceryCheckedItems[itemKey];
                              ...
                              <div
                                ...
                                className={`flex items-center justify-between p-2 rounded-xl text-xs cursor-pointer transition-all ${
                                  isChecked
                                    ...
                                    : 'bg-surface-3/60 hover:bg-surface-3 text-neutral-200 border border-border/30'
                                }`}
                              >
                                <div className="flex items-center gap-2.5 min-w-0">
                                  ...
                                  <span className={`font-medium truncate ${isChecked ? 'line-through opacity-75 text-text-muted' : ''}`}>
                                    {item.nameBg || item.name}
                                  </span>
                                </div>
                                <span className="font-mono text-[11px] font-bold text-white flex-shrink-0 ml-2">
                                  {item.totalGrams >= 1000 ? `${(item.totalGrams / 1000).toFixed(2)} кг` : `${item.totalGrams} ${item.unit || 'г'}`}
                                </span>
                              </div>
                            );
                          })}
          ...
          {/* 2. ALL WEEKLY MEALS WITH DIRECT RECIPE & INGREDIENT CHECKLIST */}
